  ...
  )
{
  ...
  if (BlockAddress == 0 && BlkIo != NULL) {
    ...
    if (ValidMBR (Mbr, BlkIo)) {
      PrintToken (STRING_TOKEN (STR_DEBUG_VALID_MBR), HiiMemHandle);
      ...
      FatType = _ValidBPB ((EFI_FAT_BOOT_SECTOR *) Buffer, BlockSize);
      if (FatType != FatUndefined) {
        _DumpBpb (FatType, (EFI_FAT_BOOT_SECTOR *) Buffer, BlockSize);
      }
    }
  }
  ...
  if (!Mbr) {
    ...
    EFIMemTableHeaderPrint (Buffer, BlockSize, BlockAddress, IsBlkDevice);
  }
}
  ...
  )
{
  ...
  Hdr.RawData = Buffer;
  if (_IsValidEfiHeader (ByteSize, Hdr)) {
    for (Index = 0; TableHeader[Index].Signature != 0x00; Index++) {

      if (TableHeader[Index].Signature == Hdr.Hdr->Signature) {
        PrintToken (
        ...
        PrintToken (
          ...
          HiiMemHandle,
          TableHeader[Index].String,
          Hdr.Hdr->HeaderSize,
          Hdr.Hdr->Revision
          );
        TableHeader[Index].Func (Hdr);
        return ;
      }
    }
    //
  ...
  )
{
  ...
  if (!EFI_ERROR (Status) && DevicePath) {
    PrintToken (STRING_TOKEN (STR_DEBUG_CONSOLE_IN_ON), HiiMemHandle, LibDevicePathToStr (DevicePath));
  }

  Status = BS->HandleProtocol (Tbl.Sys->ConsoleOutHandle, &gEfiDevicePathProtocolGuid, (VOID *) &DevicePath);
  if (!EFI_ERROR (Status) && DevicePath) {
    PrintToken (STRING_TOKEN (STR_DEBUG_CONSOLE_OUT_ON), HiiMemHandle, LibDevicePathToStr (DevicePath));
  }
  ...
  )
{
  ...
  PrintToken (STRING_TOKEN (STR_DEBUG_FAT_BPB), HiiMemHandle, FatNumber[FatType]);
  if (FatType == FAT32) {
    PrintToken (
  ...
  PrintToken (STRING_TOKEN (STR_DEBUG_SECTORSIZE), HiiMemHandle, Bpb->Fat.SectorSize, Bpb->Fat.SectorsPerCluster);
  PrintToken (
  ...
  if (FatType == FAT32) {
    PrintToken (STRING_TOKEN (STR_DEBUG_SECTORS), HiiMemHandle, Sectors, Bpb->Fat32.LargeSectorsPerFat);
  } else {
    PrintToken (STRING_TOKEN (STR_DEBUG_SECTORS), HiiMemHandle, Sectors, Bpb->Fat.SectorsPerFat);
  }

  PrintToken (STRING_TOKEN (STR_DEBUG_SECTORSPERTRACK), HiiMemHandle, Bpb->Fat.SectorsPerTrack, Bpb->Fat.Heads);
